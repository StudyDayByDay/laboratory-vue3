<template>
    <div class="gant">
      <div class="gray">活动时限：{{startDate}} ~ {{endDate}}</div>
      <a-tabs v-model:activeKey="activeKey">
        <a-tab-pane key="planTable" tab="计划表"/>
        <a-tab-pane key="planChart" tab="计划图"/>
      </a-tabs>
      <!-- is要使用字符串的形式来控制组件的话，就需要把组件注册。不注册就直接用这种对象的方式 -->
      <component :is="activeKey === 'planTable' ? planTable : planChart" :formData="formData" :startDate="startDate" :endDate="endDate"></component>
    </div>
  </template>
...
const startDate = '2022-07-06';
const endDate = '2025-08-27';
const formData = reactive([
  {id: 1, icon: 'iconMe-chajian1', planStart: dayjs('2022-07-06', 'YYYY-MM-DD'), planEnd: dayjs('2022-08-14', 'YYYY-MM-DD'), actualStart: dayjs('2022-07-06', 'YYYY-MM-DD'), actualEnd: dayjs('2022-07-31', 'YYYY-MM-DD'), chargeMan: '主题选定', role: undefined},
...
console.log(dayjs('2022-07-06', 'YYYY-MM-DD'), '时间');
</script>
